...
import org.apache.hadoop.hbase.TestClusterRule;
...
public class TestMetaBrowser {
  ...
  @ClassRule
  public static final TestClusterRule testClusterRule = new TestClusterRule();

  private final ConnectionRule connectionRule =
    new ConnectionRule(testClusterRule::createConnection);
  ...
  public void noFilters() {
    ...
    final List<RegionReplicaInfo> rows = IterableUtils.toList(new MetaBrowser(connection, request));
  ...
  public void paginateWithReplicas() {
    ...
    final List<RegionReplicaInfo> rows1 =
      IterableUtils.toList(new MetaBrowser(connection, request1));
    ...
    final List<RegionReplicaInfo> rows2 =
      IterableUtils.toList(new MetaBrowser(connection, request2));
    rows2.forEach(System.out::println);
    ...
    final List<RegionReplicaInfo> rows3 =
      IterableUtils.toList(new MetaBrowser(connection, request3));
    rows3.forEach(System.out::println);
  ...
  static class MockRequestBuilder {
    ...
    public MockRequestBuilder setStart(final Optional<String> value) {
      value.ifPresent(val -> this.start = val);
      return this;
    }
